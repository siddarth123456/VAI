...
files.forEach(file => {
  ...
  content = content.replace(/stroke="var\(--color-brand-yellow\)"/g, 'stroke="var(--color-brand-yellow)"');
  
  // Some buttons had `bg-white text-bg-dark`
  content = content.replace(/bg-white text-bg-dark/g, 'bg-black text-white');
  // Some had `text-bg-dark text-black text-xs` -> `text-white text-black text-xs` -> wait, I'll fix this manually if needed.
